				...
				if (iTarget && IsClientInGame(iTarget)) {
					...
					switch (g_iTargetType[iClient]) {
						case TYPE_GAG, TYPE_MUTE, TYPE_SILENCE: {
							PrintToChat2(iClient, "%T", "No access mute", iClient, sName);
						}
						case TYPE_UNGAG, TYPE_UNMUTE, TYPE_UNSILENCE: {
							PrintToChat2(iClient, "%T", "No access un mute", iClient, sName);
						}
					}
				} else {
					if (!iTarget) {
						switch (g_iTargetType[iClient]) {
							case TYPE_GAG, TYPE_MUTE, TYPE_SILENCE: {
								PrintToChat2(iClient, "%T", "No access mute", iClient, g_sTarget[iClient][TNAME]);
							}
							case TYPE_UNGAG, TYPE_UNMUTE, TYPE_UNSILENCE: {
								PrintToChat2(iClient, "%T", "No access un mute", iClient, g_sTarget[iClient][TNAME]);
							}
						}
					}
				}
...
void CreateDB(int iClient, int iTarget, char[] sSteamIp = "", int iTrax = 0,  Transaction hTxn = null)
{
	...
	char sQuery[1524], sLog[1024], sLength[64];
	char sAdmin_SteamID[MAX_STEAMID_LENGTH], sAdminName[MAX_NAME_LENGTH];
	char sQueryAdmin[512], sQueryTime[226], sServer[256];
			...
			if (g_iTargetType[iClient] <= TYPE_BANIP || g_iTargetType[iClient] == TYPE_ADDBAN && iTarget) {
				g_dDatabase.Format(sQuery, sizeof(sQuery), "\
						...
						VALUES (%d, '%s', '%s', '%s', UNIX_TIMESTAMP(), UNIX_TIMESTAMP() + %d, %d, '%s', %!s, '%s', %!s, ' ')", \
							g_sDatabasePrefix, iTyp, g_sTarget[iClient][TIP], g_sTarget[iClient][TSTEAMID], g_sTarget[iClient][TNAME], iTime, iTime, \
								g_sTarget[iClient][TREASON], sQueryAdmin, sAdmin_SteamID, sServer);
				...
				FormatEx(sLog, sizeof(sLog), "\"%L\" %s banned \"%s (%s IP_%s)\" (minutes \"%d\") (reason \"%s\")", \
						(g_iTargetType[iClient] == TYPE_ADDBAN) ? "add" : "", iClient, g_sTarget[iClient][TNAME], \
				...
				g_dDatabase.Format(sQuery, sizeof(sQuery), "\
						...
						VALUES (%d, '%s', '%s', UNIX_TIMESTAMP(), UNIX_TIMESTAMP() + %d, %d, '%s', %!s, '%s', %!s, ' ')", \
							g_sDatabasePrefix, iTyp, (iTyp) ? sSteamIp : "", (!iTyp) ? sSteamIp : "", \
								iTime, iTime, g_sTarget[iClient][TREASON], sQueryAdmin, sAdmin_SteamID, sServer);
				...
				FormatEx(sLog, sizeof(sLog), "\"%L\" add banned \"%s\" (minutes \"%d\") (reason \"%s\")", iClient, sSteamIp, \
							g_iTarget[iClient][TTIME], g_sTarget[iClient][TREASON]);
			}
		}
		case TYPE_GAG, TYPE_MUTE, TYPE_SILENCE: {
			int iType;
			bool bSetQ = true, bSet = true;
...
public void VerifyBan(Database db, DBResultSet dbRs, const char[] sError, any data)
{
	...
	char sSteamID[MAX_STEAMID_LENGTH], sIP[MAX_IP_LENGTH];
...
{
	char sQuery[1204], sServer[256];

	if (g_iIgnoreMuteServer) {
		if (g_iServerID == -1) {
			g_dDatabase.Format(sServer, sizeof(sServer), \
				"IFNULL ((SELECT `sid` FROM `%s_servers` WHERE `ip` = '%s' AND `port` = '%s' LIMIT 1), 0)", \
					g_sDatabasePrefix, g_sServerIP, g_sServerPort);
		} else {
			IntToString(g_iServerID, sServer, sizeof(sServer));
	...
	switch (g_iIgnoreMuteServer) {
		...
		case 1: {
			Format(sServer, sizeof(sServer), " AND `sid` = %s", sServer);
		}
		case 2: {
			Format(sServer, sizeof(sServer), " AND (`sid` = %s OR `sid` = 0)", sServer);
		}
	}
...
public void OverridesDone(Database db, DBResultSet dbRs, const char[] sError, any iData)
{
	...
	if (!dbRs || sError[0]) {
		...
		char sFlags[32], sName[256], sType[16];
...
public void GroupsDone(Database db, DBResultSet dbRs, const char[] sError, any iData)
{
	...
	if (!dbRs || sError[0]) {
		...
		char sGrpName[256], sGrpFlags[32];
		...
		int iGid = -1, iOverrideCount = 0, iOverridePos = 0;

		char szOverrideText[256], szOverrideType[4], szOverrideRule[4];
...
public void AdminsDone(Database db, DBResultSet dbRs, const char[] sError, any iData)
{
	if (!dbRs || sError[0]) {
		...
		char sIdentity[66], sPassword[66], sGroups[256];
		char sFlags[32], sName[66];

		#if MADEBUG
			int iAdmCount;
		#endif

		int iImmunity, iExpire, iExtraflags, iExtraflagsGroup;
...
void SetBdReport(int iClient, const char[] sReason)
{
	...
	char sReportName[MAX_NAME_LENGTH], sQuery[1024], sReport_SteamID[MAX_STEAMID_LENGTH];
	char sReportIp[MAX_IP_LENGTH], sSteamID[MAX_STEAMID_LENGTH], sIp[MAX_IP_LENGTH];
	char sName[MAX_NAME_LENGTH], sServer[256], sModId[256];
