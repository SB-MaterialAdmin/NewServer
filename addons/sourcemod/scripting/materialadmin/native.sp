...
public int Native_Log(Handle plugin, int numParams)
{
	...
	switch (iType) {
		case MA_LogAdmin: {
			LogToFile(g_sLogAdmin, sBufer);
		}
		case MA_LogConfig: {
			LogToFile(g_sLogConfig, sBufer);
		}
		case MA_LogDateBase: {
			LogToFile(g_sLogDateBase, sBufer);
		}
		case MA_LogAction: {
			LogToFile(g_sLogAction, sBufer);
		}
	}
